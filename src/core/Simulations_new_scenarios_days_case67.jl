...
result_expected_feasibility_checks_24_ac   = run_feasibility_checks_per_hour(test_case_bs_mn_forecasted,results_one_topology_sp_forecasted_24,ACPPowerModel,ipopt,switches_couples_ac,extremes_ZILs_ac,test_case_opf,s,n_scenarios)
result_expected_feasibility_checks_24_lpac = run_feasibility_checks_per_hour(test_case_bs_mn_forecasted,results_one_topology_sp_forecasted_24,LPACCPowerModel,ipopt,switches_couples_ac,extremes_ZILs_ac,test_case_opf,s,n_scenarios)
...
for day in 1:n_days
    ...
    #for hour in 1:n_hours_per_day
        println("day $day, first_n $first_n, last_n $last_n")
        daily_grid["nw"]   = Dict{String,Any}()
        daily_grid["multinetwork"] = true
        daily_grid["per_unit"] = true
        count_hour = 0
        for i in 1:24
            count_hour += 1
...
open(joinpath(results_folder,case,"24_hours_BS_one_topology_forecasted_$(first_hour)_$(last_hour)_day_$day.json"),"w") do f 
...
function run_simulation_days(input_dict,result_dict,name_result_file)
    for day in 1:n_days
        ...
        open(joinpath(results_folder,case,"$(name_result_file)_$(first_hour)_$(last_hour)_day_$(day).json"),"w") do f 
