...
gurobi_opf = JuMP.optimizer_with_attributes(Gurobi.Optimizer,"time_limit" => 900)#,"BarHomogeneous" => 1,"BarQCPConvTol"=>1e-4,"QCPDual" => 1, "ScaleFlag"=>2, "NumericFocus"=>2) 
...
function run_stochastic_acdcsw_AC_ZIL_per_hour(grid, model, optimizer, n_hours, n_scenarios; setting = s)
    ...
    for hour in 1:n_hours*n_scenarios
        result["$hour"] = Dict{String,Any}()
        result["$hour"] = _PMTP.run_acdcsw_AC_grid_big_M(grid["nw"]["$hour"],model,optimizer; setting = setting) 
    end
    return result
end

function run_feasibility_checks_per_hour(grid, result_bs, model, optimizer,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result_opf)
    ...
    for hour in 1:length(grid["nw"])
        ...
        result_feasibility_checks["$hour"] = _PM.solve_opf(feasibility_check,LPACCPowerModel,gurobi_opf; setting = s)
...
for hour in 1:(n_hours*one_scenario)
    result["$hour"] = _PM.solve_opf(test_case_opf_replicate["nw"]["$hour"],LPACCPowerModel,gurobi_opf; setting = s)
end
obj = [result["$i"]["objective"] for i in 1:n_hours]

result_measured = Dict{String,Any}()
for hour in 1:(n_hours*one_scenario)
    result_measured["$hour"] = _PM.solve_opf(test_case_opf_mn_measured["nw"]["$hour"],LPACCPowerModel,gurobi_opf; setting = s)
end
obj_measured = [result_measured["$i"]["objective"] for i in 1:n_hours]

result_forecasted = Dict{String,Any}()
for hour in 1:(n_hours*one_scenario)
    result_forecasted["$hour"] = _PM.solve_opf(test_case_opf_mn_forecasted["nw"]["$hour"],LPACCPowerModel,gurobi_opf; setting = s)
end
obj_forecasted = [result_forecasted["$i"]["objective"] for i in 1:n_hours]

##############################################################

result_bs_hourly = run_stochastic_acdcsw_AC_ZIL_per_hour(test_case_bs_replicate,LPACCPowerModel,gurobi,n_hours,one_scenario)
result_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate,result_bs_hourly,LPACCPowerModel,gurobi_opf,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result)
...
result_bs_hourly_measured = run_stochastic_acdcsw_AC_ZIL_per_hour(test_case_bs_replicate_mn_measured,LPACCPowerModel,gurobi,n_hours,one_scenario)
result_measured_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate_mn_measured,result_bs_hourly_measured,LPACCPowerModel,gurobi_opf,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result_measured)
...
result_bs_hourly_forecasted = run_stochastic_acdcsw_AC_ZIL_per_hour(test_case_bs_replicate_mn_forecasted,LPACCPowerModel,gurobi,n_hours,one_scenario)
result_forecasted_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate_mn_forecasted,result_bs_hourly_forecasted,LPACCPowerModel,gurobi_opf,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result_forecasted)

obj_bs_forecasted = [result_bs_hourly_forecasted["$i"]["objective"] for i in 1:n_hours]
sum(obj_bs_forecasted)

obj_fc_forecasted = [result_forecasted_feasibility_checks["$i"]["objective"] for i in 1:n_hours]
sum(obj_fc_forecasted)

[result_bs_hourly_forecasted["$i"]["solution"]["switch"]["1"]["status"] for i in 1:n_hours]
...
result_bs_hourly_one_topology            = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate              ,LPACCPowerModel,gurobi; setting = s)
...
result_bs_hourly_forecasted_one_topology = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_mn_forecasted,LPACCPowerModel,gurobi_opf; setting = s)
...
fc_one_topology = _PM.solve_mn_opf(fc_test_case_bs_replicate_auxiliary,LPACCPowerModel,gurobi_opf)
...
fc_one_topology_measured = _PM.solve_mn_opf(fc_test_case_bs_replicate_mn_measured_auxiliary,LPACCPowerModel,gurobi_opf)
...
fc_one_topology_forecasted = _PM.solve_mn_opf(fc_test_case_bs_replicate_auxiliary,LPACCPowerModel,gurobi_opf)
...
test_case_bs_replicate_sp               = deepcopy(test_case_bs_replicate              )
test_case_bs_replicate_mn_measured_sp   = deepcopy(test_case_bs_replicate_mn_measured  )
test_case_bs_replicate_mn_forecasted_sp = deepcopy(test_case_bs_replicate_mn_forecasted)
prepare_starting_value_dict_lpac_nw_sp(test_case_bs_replicate_sp           ,start_hour_simulation,end_hour_simulation,one_scenario)
prepare_starting_value_dict_lpac_nw_sp(test_case_bs_replicate_mn_measured_sp  ,start_hour_simulation,end_hour_simulation,one_scenario)
prepare_starting_value_dict_lpac_nw_sp(test_case_bs_replicate_mn_forecasted_sp,start_hour_simulation,end_hour_simulation,one_scenario)

result_bs_hourly_one_topology            = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_sp              ,LPACCPowerModel,gurobi; setting = s)
result_bs_hourly_measured_one_topology   = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_mn_measured_sp  ,LPACCPowerModel,gurobi_opf; setting = s)
result_bs_hourly_forecasted_one_topology = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_mn_forecasted_sp,LPACCPowerModel,gurobi_opf; setting = s)
...
result_bs_hourly_one_topology_sp_opf            = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_sp_opf              ,LPACCPowerModel,gurobi_opf; setting = s)
result_bs_hourly_measured_one_topology_sp_opf   = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_mn_measured_sp_opf  ,LPACCPowerModel,gurobi_opf; setting = s)
result_bs_hourly_forecasted_one_topology_sp_opf = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology(test_case_bs_replicate_mn_forecasted_sp_opf,LPACCPowerModel,gurobi_opf; setting = s)
