...
gurobi_opf = JuMP.optimizer_with_attributes(Gurobi.Optimizer,"time_limit" => 1800)#,"BarHomogeneous" => 1,"BarQCPConvTol"=>1e-4,"QCPDual" => 1, "ScaleFlag"=>2, "NumericFocus"=>2) 
...
for sw_id in 1:length(extremes_ZILs_ac)
    test_case_bs["switch"]["$sw_id"]["cost"] = 50.0
end
...
for hour in 1:(n_hours*one_scenario)
    result["$hour"] = _PM.solve_opf(test_case_opf_replicate["nw"]["$hour"],ACPPowerModel,ipopt; setting = s)
end
...
for hour in 1:(n_hours*one_scenario)
    result_measured["$hour"] = _PM.solve_opf(test_case_opf_mn_measured["nw"]["$hour"],ACPPowerModel,ipopt; setting = s)
end
...
for hour in 1:(n_hours*one_scenario)
    result_forecasted["$hour"] = _PM.solve_opf(test_case_opf_mn_forecasted["nw"]["$hour"],ACPPowerModel,ipopt; setting = s)
end
...
result_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate,result_bs_hourly,ACPPowerModel,ipopt,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result)
...
result_measured_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate_mn_measured,result_bs_hourly_measured,ACPPowerModel,ipopt,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result_measured)
...
result_forecasted_feasibility_checks = run_feasibility_checks_per_hour(test_case_bs_replicate_mn_forecasted,result_bs_hourly_forecasted,ACPPowerModel,ipopt,switches_couples_ac,extremes_ZILs_ac,test_case_opf,result_forecasted)
...
result_bs_hourly_one_topology            = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology_sp(test_case_bs_replicate_sp              ,LPACCPowerModel,gurobi; setting = s)
...
result_bs_hourly_forecasted_one_topology = _SPMTA.run_stochastic_acdcsw_AC_ZIL_one_topology_sp(test_case_bs_replicate_mn_forecasted_sp,LPACCPowerModel,gurobi_opf; setting = s)
...
test_case_bs_1 = deepcopy(test_case_bs_replicate_mn_forecasted_sp)
test_case_bs_1["total_switching_actions"] = 1
test_case_bs_1_result = _SPMTA.run_stochastic_acdcsw_AC_ZIL_limit_switching_actions_sp(test_case_bs_1,LPACCPowerModel,gurobi_opf)

[test_case_bs_1_result["solution"]["nw"]["$i"]["switch"]["1"]["status"] for i in 1:n_hours]

test_case_bs_2 = deepcopy(test_case_bs_replicate_mn_forecasted_sp)
test_case_bs_2["total_switching_actions"] = 2
test_case_bs_2_result = _SPMTA.run_stochastic_acdcsw_AC_ZIL_limit_switching_actions_sp(test_case_bs_2,LPACCPowerModel,gurobi_opf)

[test_case_bs_2_result["solution"]["nw"]["$i"]["switch"]["1"]["status"] for i in 1:n_hours]


test_case_bs_3 = deepcopy(test_case_bs_replicate_mn_forecasted_sp)
test_case_bs_3["total_switching_actions"] = 3
test_case_bs_3_result = _SPMTA.run_stochastic_acdcsw_AC_ZIL_limit_switching_actions_sp(test_case_bs_3,LPACCPowerModel,gurobi_opf)

[test_case_bs_3_result["solution"]["nw"]["$i"]["switch"]["1"]["status"] for i in 1:n_hours]
