...
open(joinpath(results_folder,case,"forecasted_wind.json"),"w") do f 
    write(f, json_forecasted_wind) 
end 

plot(forecasted_wind,label = :none,ylims = (0,1.0),xlabel = "Hours", ylabel = "Capacity factor [-]",grid = :none,xticks = (1:1:24))
#########################################################################################
## Running simulations
# Busbar splitting
