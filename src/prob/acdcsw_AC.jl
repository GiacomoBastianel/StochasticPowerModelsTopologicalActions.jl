export run_stochastic_acdcsw_AC
...
function run_stochastic_acdcsw_AC(file, model_constructor, optimizer; kwargs...)
    _FP.require_dim(file, :hour, :scenario)
    return _PM.solve_model(file, model_constructor, optimizer, build_stochastic_acdcsw_AC; 
...
""
function build_stochastic_acdcsw_AC(pm::_PM.AbstractPowerModel)
    # AC grid
    ...
    # Objective function
    objective_stochastic_switch(pm)
    ...
    for n in _FP.nw_ids(pm)
        ...
        for i in _PM.ids(pm, n, :switch)
            ...
            _PMTP.constraint_switch_voltage_on_off(pm, i; nw = n) # making sure that the voltage magnitude and angles are equal at the two extremes of a closed switch
    ...
    for n in _FP.nw_ids(pm)
        for i in _PM.ids(pm, :switch, nw=n)
            constraint_switching_binaries(pm, n, i)
        end
    end
